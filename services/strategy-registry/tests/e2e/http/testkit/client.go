...
func (c Client) IdentityPost(t *testing.T, path string, body any) Response {
	t.Helper()
	return c.doJSON(t, http.MethodPost, c.env.IdentityURL+"/api"+path, nil, body)
}

func (c Client) IdentityDelete(t *testing.T, path string) Response {
	t.Helper()
	return c.doJSON(t, http.MethodDelete, c.env.IdentityURL+"/api"+path, nil, nil)
}

func (c Client) doJSON(t *testing.T, method, reqURL string, query url.Values, body any) Response {
	t.Helper()
...
func GraphPath(strategyID string) string {
	return StrategyPath(strategyID) + "/graph"
}
