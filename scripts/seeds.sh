...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/seeds.sh [services] [parallel]
  ./scripts/seeds.sh -s strategy-registry
  ./scripts/seeds.sh --services "identity"

Options:
  -s, --service NAME       Add one service (repeatable)
      --services LIST      Comma or space separated services
  -p, --parallel           Enable parallel mode for seeds
...
if [[ "${#services[@]}" -eq 0 ]]; then
  echo "No services provided for seeds."
  exit 1
fi
...
echo "Running seeds for: $display_services"

seed_cmd_for_service() {
  local svc="$1"
  if [[ -f "./services/$svc/cmd/ctl/main.go" ]]; then
...
run_seed() {
  ...
  local cmd
  local cmd_status=0
  if ! cmd="$(seed_cmd_for_service "$svc")"; then
    {
      printf "[%s] seed\n" "$svc"
      seed_cmd_for_service "$svc"
      printf "\n"
    } >"$output_file" 2>&1
    return 1
  fi
  ...
  {
    printf "[%s] seed\n" "$svc"
    LOG_LEVEL=error go run "$cmd" seed || cmd_status=$?
    printf "\n"
  } >"$output_file" 2>&1
  return "$cmd_status"
}
...
if [[ "$status" -eq 0 ]]; then
  printf "%sSeeds completed successfully.%s\n" "$c_green" "$c_reset"
  exit 0
fi

printf "%s[ERROR]%s Seeds failed.\n" "$c_err_badge" "$c_badge_reset"
exit "$status"
